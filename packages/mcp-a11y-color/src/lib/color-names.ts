...
import { deltaE } from './color-blindness.js';

type NamedColor = { name: string; rgb: RGBColor };
...
export const findNearestColorName = (color: RGBColor): NearestColorResult => {
  let bestMatch: NamedColor = CSS_NAMED_COLORS[0];
  let bestDelta = Number.POSITIVE_INFINITY;

  for (const entry of CSS_NAMED_COLORS) {
    const d = deltaE(color, entry.rgb);
    if (d < bestDelta) {
      bestDelta = d;
      bestMatch = entry;
      if (d === 0) break;
  ...
  const hex = `#${componentToHex(bestMatch.rgb.r)}${componentToHex(bestMatch.rgb.g)}${componentToHex(bestMatch.rgb.b)}`;

  return {
    name: bestMatch.name,
    hex,
    rgb: bestMatch.rgb,
    deltaE: Math.round(bestDelta * 10) / 10,
    exact: bestDelta === 0,
  };
};

export const findNearestColorNames = (color: RGBColor, count: number): NearestColorResult[] => {
  const scored = CSS_NAMED_COLORS.map((entry) => ({
    entry,
    dist: deltaE(color, entry.rgb),
  }));

  scored.sort((a, b) => a.dist - b.dist);

  return scored.slice(0, count).map(({ entry, dist }) => ({
    name: entry.name,
    hex: `#${componentToHex(entry.rgb.r)}${componentToHex(entry.rgb.g)}${componentToHex(entry.rgb.b)}`,
    rgb: entry.rgb,
    deltaE: Math.round(dist * 10) / 10,
    exact: dist === 0,
  }));
};
